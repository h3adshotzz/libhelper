...
/**
 *  \brief      Redefinition of ident load command as a libhelper type.
 *
 *              NOTE:   This Load Command is marked obsolete by Apple, therefore is
 *                      not supported by the libhelper-macho parser.
 */
typedef struct ident_command                       mach_ident_command_t;

/////////////////////////////////////////////////////////////////////////////////

/**
 *  \brief      Redefinition of symseg load command as a libhelper type.
 *
 *              NOTE:   This Load Command is marked obsolete by Apple, therefore is
 *                      not supported by the libhelper-macho parser.
 */
typedef struct symseg_command                       mach_symseg_command_t;
...
/**
 *  \brief      Get a formatted string representing a Dynamic Libraries current or
...
/**
 *  \brief      Unpack a UUID string from an `LC_UUID` Load Command and return the 
 *              result as a formatted string.
 *
 *  \param uuid     The UUID Load Command.
 *
 *  \returns    Returns a formatted string for the unpacked UUID, or NULL if the 
 *              operation failed.
 */
extern char *
...
/**
 *  \brief      A build version load command info structure. A mach_build_version_command_t
 *              can be parsed into this info structure which contains strings for the
 *              build platform, minimum OS version and SDK version, along with an array of
 *              build tools that were used.
 */
...
/**
 *  \brief      Parse an LC_BUILD_VERSION Load Command, unpack platform, minimum os and
 *              sdk version strings and tools used for compiling the binary. 
 *
 *  \param bvc      The Build Version Load Command.
 *
 *  \returns    Returns a `mach_build_version_info_t`, or NULL if the operation failed.
 */
 ...
 * 
 *              There are no helper functions for this load command. The `entry_id`
